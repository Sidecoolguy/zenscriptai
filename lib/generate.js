...
VALID GPC HARD REQUIREMENTS (these are mandatory, not suggestions):
- Output exactly ONE top-level function named main that runs forever (while(true) { ... }).
- Use ONLY the GPC functions available to the include file: set_val, get_val, wait, and the constants defined at the top. Never invent any other function or API name.
- All braces and parentheses must be balanced. Every int variable you use must be declared at the top before main().
- Use integer literals only (no floats) for wait() and set_val(). Keep wait() values between 1 and 5000.
- Never emit explanatory text, pseudo-code, or a placeholder. If a requested mod has no effect on the target, still emit a minimal, fully valid, compilable main() rather than returning nothing.
`;
